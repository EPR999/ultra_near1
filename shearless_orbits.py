...
def action2(x_cross):
    ...
    ax.plot(points[0].real , points[1].real , points[0].imag,  "-o",color = colormap(3 * j+ 1))
...
def action2_orbit(thetas):
    ...
    for j in range(len(thetas)):
        ...
        p = b_orb * np.cos(theta)
        qp = np.array([q,p])
        x_cross = np.array([q * np.cos(t) - p * np.sin(t),  q * np.sin(t) + p * np.cos(t)])
        S = 0   
        i = 0
        x_cross0 = x_cross
        prepoint  = x_cross
        ...
        while i <  3800:
            ...
            #print('x_cross = ',x_cross)
            #print(np.linalg.norm(x_cross - periodicpoint))    
        ax2.plot(iterates,actions,"-o",color = colormap(3 * j + 1))
        #ax2.plot(iterates,actions,"-o",color = "red")
        ax2.plot(iterates[0],actions[0],"ro",markersize = 12)
        periodicpoint = np.loadtxt("complex_periodicpoint_12.txt",dtype = np.complex128)
        ax.plot(periodicpoint[:,0].real,periodicpoint[:,1].real,periodicpoint[:,0].imag,"o",color = "purple",markersize = 10)
        print(iterates.shape)
        #ax.plot(points[0].real , points[1].real , points[0].imag,  "-o",color = colormap(4 * j  + 1))
        ax.plot(points[0].real , points[1].real , points[0].imag,  "-o",color = colormap(3 * j ))
        ax.plot(initpoint[:,0].real,initpoint[:,1].real,initpoint[:,0].imag,"ro",markersize = 12)
        #boolean = np.abs(points[0].imag ) < 0.1
        boolean = ( np.diff( np.sign(points[0].imag)  ) != 0 )
        boolean = np.append(boolean,False)
        ...
        #ax.set_title("xi = {},eta = {}".format(xi_str, eta_str))
        if lambda_1 == 3.0:
              ax2.set_ylim(-100,380)
        ax_proj.plot(phase[0],phase[1],",",color = colormap(0),zorder = 0)
        ax_proj.plot(initpoint[:,0].real,initpoint[:,1].real,"ro",markersize = 12)
        ax2.plot(iterates,distancefrom_real,"-o",color = colormap(3 * j +  1))
        ...
        #ax_proj.plot(iterates,distancefrom_real,"ro",markersize= 12,zorder = 100)
        for i in range(len(near_real[0])):
            ax_proj.plot(near_real[0][i].real , near_real[1][i].real, "o",color = colormap(2),markersize = 10,zorder = 2)
            #ax.plot(near_real[0].real , near_real[1].real, near_real[0].imag,color = "o",markersize = 10)
            ax2.plot(iterates[boolean][i],actions[boolean][i],"o",markersize = 10,color = colormap(2))
        ...
        for i in range(len(iterates_str)):
            ax2.text(iterates_bool[i]+1,actions_bool[i] + 1,iterates_str[i],fontsize = 20)
        print(near_real[:,0])
        ax_proj.plot(points[0].real, points[1].real, "-o",color = colormap(j* 3 + 1 ),zorder = 1)
    plt.show()


#textpath = 'initpoint_10_13_test14_a_{}_b_{}.txt'.format(str(round(a_orb,3)).replace('.','z'), str(round(b_orb,3)).replace('.','z'))
textpath = 'initpoint_10_13_test14_a_{}_b_{}.txt'.format(str(round(a_orb,3)).replace('.','z'), str(round(b_orb,3)).replace('.','z'))
theta  = np.loadtxt("orbit_a0654_b1533_189_14.txt")
theta  = np.loadtxt("orbit_a0654_b1533_63_14.txt")
#action2_orbit(np.array([3.03957 + 2.868963j, 3.038237  + 2.886782j]))
#xi1 = 3.201024,eta1=3.092145
#xi2=3.345059,eta2=3.192153orbit_a0654_b1533_157_14
#action2_orbit(np.array([3.201024 + 3.092145 * 1j, 3.34059  + 3.192153 * 1j]))
plt.rcParams["font.size"] = 30
#plt.xlim(0,2 * np.pi)
plt.tick_params(labelsize=20)
fig = plt.figure(figsize = (16,12))
...
ax2.set_xlabel("Time step",fontsize = 27)
ax.set_xlabel(r"${\rm Re}(q)$",labelpad = 20)
ax.set_ylabel(r"${\rm Re}(p)$",labelpad = 20)
ax.set_zlabel(r"${\rm Im}(q)$",labelpad = 30)
#plt.ylabel("Im(S)",fontsize = 27)
...
#for i in range(1,50,10):
    #action2_orbit(np.array([3.201024 + 3.092145 * 1j]))
#    action2_orbit(np.array([theta[i,0] + theta[i,1] * 1j]))
#action2_orbit(np.array([2.103203 + 2.621572 * 1j,2.066582+  2.75574 * 1j]))
#action2_orbit(np.array([2.103203 + 2.621572 * 1j]))
#i = 3.110982,eta = 3.384384
#action2_orbit(np.array([3.110982 + 3.384384 * 1j]))
#action2_orbit(np.array([3.121011 + 3.570003 * 1j]))
...
#plt.show()
#action2_orbit(np.array([2.103203 + 2.621572 * 1j,2.142629+  2.703767 * 1j]))
#action2_orbit(np.array([2.103203 + 2.621572 * 1j]))
#action2_orbit(np.array([2.098643 +  2.645964 * 1j]))
action2_orbit(np.array([2.065383 +  2.688213 * 1j]))
#action2_orbit(np.array([2.098643 +  2.645964 * 1j]))
#action2_orbit(np.array([2.103203 + 2.621572 * 1j]))
#plt.rcParams["font.size"] = 30
#plt.xlim(0,2 * np.pi)
...
#ax.set_ylabel(r"${\rm Re}(p)$",labelpad = 20)
#ax.set_zlabel(r"${\rm Im}(q)$",labelpad = 30)
#action2_orbit(np.array([2.142629 +  2.703767 * 1j]))
#action2_orbit(np.array([2.103203+  2.621572 * 1j ]))
#plt.show()
