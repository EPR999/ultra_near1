...
def mset():#Julia集合を描く
    ...
    #exit(
    print(div_numbers)
    plt.scatter(xi2,eta2, s=0.1, c=div_numbers, marker = '.',cmap='seismic', vmax =step,vmin = 0,alpha = 0.8)
    cbar = plt.colorbar()
...
def mset2(x1,x2,y1,y2):
    ...
    for  i in range(step):
        ...
        if i < step3:
           ...
           #     plt.plot(xi4_mset[boolean_mset],eta4_mset[boolean_mset],',g',zorder = 3)
            if i == step3 - 1:
                boolean_mset =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ) != 0, qp_rotated_mset[0][:,:-1] < 10 ** 5)
                boolean_quantum = np.logical_and(np.abs(np.abs(qp_rotated_mset[0][:,:-1]) - quantum_q) < threshold, qp_rotated_mset[0][:,:-1] < 10 ** 5)
                boolean_mset_quantum = np.logical_and(boolean_mset,boolean_quantum)
                plt.plot(xi3_mset[boolean_mset_quantum],eta3_mset[boolean_mset_quantum],'o',zorder = 3)
                plt.plot(xi3_mset[boolean_mset],eta3_mset[boolean_mset],',k',zorder = 3)
                boolean_quantum = np.logical_and(np.abs(np.abs(qp_rotated_mset[0][:-1,:]) - quantum_q) < threshold, qp_rotated_mset[0][:-1,:] < 10 ** 5)
                boolean_mset =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) , axis = 0 ) != 0, qp_rotated_mset[0][:-1,:] < 10 ** 5)
                boolean_mset_quantum = np.logical_and(boolean_mset,boolean_quantum)
                plt.plot(xi4_mset[boolean_mset_quantum],eta4_mset[boolean_mset_quantum],'o',zorder = 3)
                plt.plot(xi3_mset[boolean_mset],eta3_mset[boolean_mset],',k',zorder = 3)
            #elif i == 126 - 1:
            #    boolean_mset =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ) != 0, qp_rotated_mset[0][:,:-1] < 10 ** 5)
    ...
    #plt.xlim(0,2 * np.pi)
    plt.tick_params(labelsize=20)
    plt.scatter(xi2,eta2, s=0.1, c=div_numbers, marker = '.',cmap='seismic', vmax = step,vmin = 0,alpha = 0.4)
    cbar = plt.colorbar()
