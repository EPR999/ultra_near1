...
def action2(x_cross):
    ...
    for i in range(len(near_real[0])-1):
        ax_proj.plot(near_real[0][i].real , near_real[1][i].real, "o", color = colormap(i),markersize = 10,zorder = 2)
        #ax.plot(near_real[0].real , near_real[1].real, near_real[0].imag,color = "o",markersize = 10)
        ax2.plot(iterates[boolean][i],actions[boolean][i],"o",markersize = 10,color = colormap(i))
...
def mset2(x1,x2,y1,y2):
    ...
    for  i in range(step):
        ...
        if i < step3:
            ...
            if i == step2 - 1 :
                boolean_mset =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ) != 0, qp_rotated_mset[0][:,:-1] < 10 ** 5)
                boolean_mset2 =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ,axis = 0) != 0, qp_rotated_mset[0][:-1,:] < 10 ** 5)
                plt.plot(xi3_mset[boolean_mset],eta3_mset[boolean_mset],',g',zorder = 3)
                print(xi3_mset.shape)
                plt.plot(xi4_mset[boolean_mset2],eta4_mset[boolean_mset2],',g',zorder = 3)
            elif i == step3 - 1:
                boolean_mset =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ) != 0, qp_rotated_mset[0][:,:-1] < 10 ** 5)
                boolean_mset2 =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag), axis = 0  ) != 0, qp_rotated_mset[0][:-1,:] < 10 ** 5)
                plt.plot(xi3_mset[boolean_mset],eta3_mset[boolean_mset],',k',zorder = 3)
                plt.plot(xi4_mset[boolean_mset2],eta4_mset[boolean_mset2],',k',zorder = 3)
           # elif i == 126 - 1: 
           #     boolean_mset =  np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ) != 0, qp_rotated_mset[0][:,:-1] < 10 ** 5)
           #     plt.plot(xi2_mset[boolean_mset],eta2_mset[boolean_mset],',r',zorder = 3)
           # elif i == 94 -1:
