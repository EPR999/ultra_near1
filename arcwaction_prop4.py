...
def action2(x_cross,periodicpoint,period,ax2):
	...
	imagq = np.array([])
	#plt.subplot(1,1,1)
	ax2.set_ylabel(r"${\rm Im}(q)$",fontsize = 27)
	ax2.set_xlabel("iterates",fontsize = 27)
	ax2.set_ylim(-0.05,0.18)
	ax2.axhline(y = 0)
	ax2.set_xlim(-1,25) 
	colormap = plt.get_cmap("tab10")
	ax2.plot(0,x_cross[0].imag,'o',color = colormap(period - 3),markersize = 12,zorder = 100 - period)
	...
	while True:
		...
		if  np.linalg.norm(x_cross - periodicpoint) < 10 ** (-6) and period !=1 or i == 23:
			...
			ax2.plot(iterates,imagq,"-o",color = colormap(period - 3),zorder = 100 - period)
			...
			#print("theta? = ",np.linalg.norm(np.imag(x_cross)))
			#plt.title("stability = {}".format(	str(round(wabs1 ** 0.5, 4))))
			ax2.plot(iterates,imagq,"-o",color = colormap(period),zorder = 1)
