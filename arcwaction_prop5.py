...
#		plt.ylim(-0.001,0.03)
 ...
 #cid = fig.canvas.mpl_connect('button_press_event', 
################################################################
def action2(x_cross,periodicpoint,period,ax2):
	...
	imagq = np.array([])
	#plt.subplot(1,1,1)
	ax2.set_ylabel(r"${\rm Im}(S)$",fontsize = 27)
	ax2.set_xlabel("Time step",fontsize = 27)
	ax2.set_ylim(-0.01,0.035)
	#ax2.axhline(y = 0)
	ax2.set_xlim(-1,16) 
	colormap = plt.get_cmap("tab10")
	#ax2.plot(0,0,'o',color = colormap(period - 3),markersize = 12,zorder = 100 - period)
	ax2.plot(0,0,'o',color = "blue",markersize = 12,zorder = 100 - period)
	t = np.linspace(0,1,10 ** 3)
	
	while True:
		iterates = np.append(iterates, i)
		actions = np.append(actions, S.imag)
		imagq = np.append(imagq, x_cross[0].imag)
		...
		if  np.linalg.norm(x_cross - periodicpoint) < 10 ** (-6) and period !=1 or i == 16: 
			#print("theta = ",np.linalg.norm(np.imag(x_cross)))
			#plt.title("stability = {}".format(str(round(wabs1 ** 0.5, 4))))
			#ax2.plot(iterates,actions,"-o",color = colormap(period - 3),zorder = 100 - period)
			ax2.plot(iterates,actions,"-o",color = "blue",zorder = 100 - period)
			...
			#print("theta? = ",np.linalg.norm(np.imag(x_cross)))
			#plt.title("stability = {}".format(	str(round(wabs1 ** 0.5, 4))))
			ax2.plot(iterates,actions,"-o",color = colormap(period),zorder = 1)
			#plt.plot(theta,,"-o",color = "red",zorder = 1)
			#ax2.plot(theta.real, theta.real ,"-o",color = "green",zorder = 1)
			print("S = ",S)
			#S -= V(prepoint)
			#print("Im(Smin)= " ,S.imag)
			#print('x_cross = ',x_cross)
...
def getsection_on_thetaplane(xi,eta,x1,x2,y1,y2):
	...
	#plt.subplot(1,2,2)
	#qp = rotateinitialmanifold(a_orb,b_orb,-t,qp)
	fig2 = plt.figure(figsize =  (14.402,8.38889))
	ax2 = fig2.add_subplot(111)
	for i in range(len(qp[1,:])):
		print(len(qp[1,:]))
		print(periods[i])
		drawingpoint(qp[:,i],periodicpoint[:,i],periods[i],ax2)
	plt.show()
	return allpoint
def Press(event):
...
def drawingpoint(point,periodicpoint,period,ax2):
	...
	print(point)
	prepoint = point
	stability = action2(pointed,periodicpoint,period,ax2)
	return
	#print(linepoints)
	#Z = f(X, Y)
	...
	for i in range(300):
		...
		if np.all(linepoints[2] > 0) == False:
			boolean_minusz = linepoints[2] > -0.000
			boolean_plusz = linepoints[2] < -0.000
			if np.all( np.abs(linepoints[2]) < 0.000 )  == False:
