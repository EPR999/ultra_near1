...
def V(x):
    return np.array(x) ** 2/2 - np.cos(x/lambda_1) 

#def V(x):
#    return - (k/16) * np.exp(-8 * (x ** 2)) - ((e2/(2 * (8 ** 0.5))) * np.sqrt(np.pi)) * (scipy.special.erf( (8 ** 0.5)* (x - xb)) - scipy.special.erf((8 ** 0.5) *( x + xb)))
...
def action2(x_cross):
    ...
    ax_proj.plot(near_real[0].real , near_real[1].real, "o",markersize = 10,zorder = 2)
    ...
    for i in range(len(iterates_str)):
        ax2.text(iterates_bool[i],actions_bool[i],iterates_str[i],fontsize = 10)
    #print(near_real[:,0])
...
def mset():#Julia集合を描く
    ...
    import copy
    actions = np.full_like(qp_rotated[0],0) + generatorfunc2(theta) 
    print(actions.imag)
    for  i in range(step):
        qp_rotated = cmap.U(qp_rotated)
        qp_rotated_pre = copy.deepcopy(qp_rotated)  
        #qp_rotated = cmap.U(qp_rotated)
        actions -=  tau * qp_rotated[1] ** 2/2 + tau *  V(qp_rotated_pre[0]) - qp_rotated[1] * (qp_rotated[0] - qp_rotated_pre[0])
        if i == step - 1:
            qp_rotated_mset = copy.deepcopy(qp_rotated)
    ...
    #print(mset2_Release)
    #exit(
    boolean = np.logical_and(np.diff( np.sign(qp_rotated_mset[0].imag) ) != 0, qp_rotated_mset[0][:,:-1] < 10 ** 5)
    plt.scatter(xi2,eta2, s=0.1, c= np.log10(np.exp(-2  * actions.imag)), marker = '.',cmap='jet_r', vmin = -300,vmax = 0,zorder = 1)
    #plt.scatter(xi2,eta2, s=0.1, c=qp_rotated_mset[0].imag, marker = '.',cmap='jet', vmin = -5,vmax = 5,zorder = 1)
    xi2 = xi2[:,:-1]
    eta2 = eta2[:,:-1]
    #plt.ylim(eta2[0,0],eta2[-1,-1])
    #plt.xlim(xi2[0,0],xi2[-1,-1])
    plt.plot(xi2[boolean],eta2[boolean],',k',zorder = 3)
    print(div_numbers)
    print(actions.imag)
    #print(qp.rotated)
    cbar = plt.colorbar()
    cbar.ax.tick_params(labelsize=20)
    cbar.set_label(r"$| {\rm Im}S|$",size = 25)
...
def mset2(x1,x2,y1,y2):
    ...
    #plt.ylim(eta2[0,0],eta2[-1,-1])
    #plt.xlim(xi2[0,0],xi2[-1,-1])
    #plt.scatter(xi2,eta2, s=0.1, c=np.abs(actions.imag), marker = '.',cmap='jet', vmax = 300,vmin = 0)
    #plt.scatter(xi2,eta2, s=0.1, c=np.abs(actions.imag), marker = '.',cmap='jet', vmin = 0,vmax = 300,zorder = 1)
    plt.scatter(xi2,eta2, s=0.1, c= np.log10(np.exp(-2 * actions.imag)), marker = '.',cmap='jet_r', vmin = -300,vmax = 0,zorder = 1)
    #plt.scatter(xi2,eta2, s=0.1, c=qp_rotated[0].imag, marker = '.',cmap='jet', vmin = -5,vmax = 5,zorder = 1)
    xi2 = xi2[:,:-1]
    eta2 = eta2[:,:-1]
    plt.plot(xi2[boolean],eta2[boolean],',k',zorder = 3)
    print(div_numbers)
