...
#[ 4.02745448 -4.02752761  4.13948534]
#[[ 4.13941222  4.13686566  4.13432233 ... 12.1791845  12.18682435
#  12.19446743]
# [ 4.13672394  4.13417577  4.13163083 ... 12.17140292 12.17904116
#  12.18668263]
# [ 4.13403898  4.1314892   4.12894264 ... 12.16362466 12.17126129
#  12.17890114]
## ...
# [12.17890114 12.17126129 12.16362466 ...  4.12894264  4.1314892
#   4.13403898]
# [12.18668263 12.17904116 12.17140292 ...  4.13163083  4.13417577
#   4.13672394]
# [12.19446743 12.18682435 12.1791845  ...  4.13432233  4.13686566
#   4.13941222]]
#[2.06892719 6.09801264] [[-0.71687012  0.69720674]
# [-0.69720674 -0.71687012]]
#[0.69522823 0.40495414]...1.0

#[ 4.01424966 -4.01425942 13.68562372]
#[[13.68561396 13.68307571 13.68054068 ... 21.69890006 21.70651483
#  21.71413281]
# [13.67084121 13.66830136 13.66576472 ... 21.67905079 21.68666395
#  21.69428033]
# [13.65607942 13.65353796 13.65099971 ... 21.65921248 21.66682404
##  21.6744388 ]
 #...
 #[21.6744388  21.66682404 21.65921248 ... 13.65099971 13.65353796
 # 13.65607942]
 #[21.69428033 21.68666395 21.67905079 ... 13.66576472 13.66830136
 # 13.67084121]
 #[21.71413281 21.70651483 21.69890006 ... 13.68054068 13.68307571
 # 13.68561396]]
#[ 3.61424775 14.08562564] [[-0.98071426  0.19544705]
# [-0.19544705 -0.98071426]]
#[0.52600641 0.26644767] ...0.3
#angle between major axis and x axis= 0.19671327037945563
#angle between minor axis and x axis= -1.374083056415441

#[ 4.01158291 -4.01160195 40.98381434]
#[[40.9837953  40.98125874 40.9787254  ... 48.99177655 48.99938627
#  49.00699919]
## [40.93448805 40.93194989 40.92941494 ... 48.93739615 48.94500426
 # 48.95261558]
 #[40.88521363 40.88267386 40.8801373  ... 48.88304856 48.89065507
 # 48.89826478]
 #...
 #[48.89826478 48.89065507 48.88304856 ... 40.8801373  40.88267386
 # 40.88521363]
 #[48.95261558 48.94500426 48.93739615 ... 40.92941494 40.93194989
 # 40.93448805]
 #[49.00699919 48.99938627 48.99177655 ... 40.9787254  40.98125874
 # 40.9837953 ]]
#[ 3.90308349 41.09231375] [[-0.99854019  0.05401385]
# [-0.05401385 -0.99854019]]
#[0.50616962 0.15599824]
#[-0.99854019 -0.05401385]...0.1
#angle between major axis and x axis= 0.05404014456267413
#angle between minor axis and x axis= -1.5167561822322224
...
def dotV(x):
    return - K * np.sin(x - np.pi) 

def V(x):
    return K * np.cos(x- np.pi)
...
class KScatteringMap:
    ...
    def ScattMapt(self,qp):
        ...
        #正の時間方向の写像
    def U(self,qp):
        qp_new = copy.deepcopy(qp)
        qp_new[0] =  np.array(qp[0] + qp[1] - dotV(qp[0]))
        qp_new[1] = np.array(qp[1] -  dotV(qp[0]))
        return np.array(qp_new)
...
def action2(x_cross):
    ...
    S = 0
    i = 0
    text  = "scatteringphase_standard_001.txt"
    x_cross0 = x_cross
    prepoint  = x_cross
    ...
    while i <100:
        ...
        S += 0.5 *  (x_cross[1] ** 2) +  V(prepoint[0]) - x_cross[1] * ( x_cross[0] - prepoint[0] ) 
        #actions = np.append(actions, S.imag)
    ...
    ax_proj.set_xlim( -np.pi,np.pi)
    ax_proj.set_ylim( -np.pi,np.pi)
    ...
    ax_proj.plot(near_real[0].real , near_real[1].real, "o",markersize = 10,color = "green",zorder = 2)
    ax_proj.plot(initpoint[:,0].real,initpoint[:,1].real,"ro",markersize = 12)
    ax.plot(near_real[0].real , near_real[1].real, near_real[0].imag,"o",markersize = 10,color = "green")
    ax2.plot(iterates[boolean],actions[boolean],"o",markersize = 10,color = "green")
    iterates_str = np.array(np.round(iterates[boolean],decimals = 0),dtype = str)
    print(iterates)
    iterates_bool = iterates[boolean]
    actions_bool = actions[boolean]
    for i in range(len(iterates_str)):
        ax2.text(iterates_bool[i]+0.1,actions_bool[i] + 0.1,iterates_str[i],fontsize = 15)
    #print(near_real[:,0])
...
def mset():#Julia集合を描く
   ...
   # p = np.zeros_like( q)
    ...
    #exit(
    print(div_numbers)
    plt.scatter(xi2,eta2, s=0.1, c=div_numbers, marker = '.',cmap='seismic', vmax =step,vmin = 0)
    cbar = plt.colorbar()
    ...
    #plotperiods(textpath)[0.69522823 0.40495414]
