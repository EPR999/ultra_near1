...
def mset():#Julia集合を描く
    ...
    div_numbers_checkbool  =  np.linalg.norm(qp_rotated,axis = 0)  < 3 #すでに発散したかどうか
    ...
    for  i in range(step):
        ...
        boolean = np.logical_or(np.linalg.norm(qp_rotated,axis = 0) > 3, np.isnan(np.linalg.norm(qp_rotated,axis = 0)) == True)#発散してるところ
