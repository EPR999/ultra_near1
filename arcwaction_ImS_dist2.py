...
def mset():
	global ax4,fig4
	originx = 0
	originy = 0#0.88#2 * np.pi
	height = 4.0#0.92 - originy
	grid = 1500
	step = 12
	grid2 = grid - 1
	width = 2 * np.pi
	...
	mapped = ScattMapt2(qp_rotated,step)
	...
	eta4 = eta2[ :-1 , :]
	#print(mset2_Release)
	ax4.set_ylim(eta2[0,0],eta2[-1,-1])
	ax4.set_xlim(xi2[0,0],xi2[-1,-1])
	ax4.plot(xi3[boolean],eta3[boolean],',k',zorder = 1)
	ax4.plot(xi4[boolean2],eta4[boolean2],',k',zorder = 1)
	ax4.set_xlabel(r"$\xi$",fontsize = 24)
	ax4.set_ylabel(r"$\eta$",fontsize = 24, rotation = 'horizontal',labelpad = 13)
...
def action2(x_cross,periodicpoint,period):
	...
	plt.ylim(-0.06,0.06)
	plt.xlim(-1,25) 
	plt.plot(0,0,'o',color = "green",markersize = 10,zorder = 2)
	while True:
		...
		if  np.linalg.norm(x_cross - periodicpoint) < 10 ** (-6) and period !=1 or i == 23:
			#print("theta = ",np.linalg.norm(np.imag(x_cross)))
			#plt.title("stability = {}".format(str(round(wabs1 ** 0.5, 4))))
			plt.plot(iterates,actions,"-o",color = "red",zorder = 1)
...
def getsection_on_thetaplane(xi,eta,x1,x2,y1,y2):
	global textpath,a_orb,b_orb,count
	...
	eta_abs = np.abs(data[boolean,11])
	xi2 = data[boolean,10]
	...
	q = data[boolean,6]
	p = data[boolean,7]
	#eta_abs = np.abs(eta_point)
	print(data)
	ImS = np.abs(data[boolean,2])
	qp = np.array([q,p])
	print(qp)
	print(count)
	if count == 0:
		mset()
	eta = np.abs(data[boolean,11])
	eta_all = np.abs(np.array(data[:,11]))
	ImS_all = np.abs(np.array(data[:,2]))
	#fig = plt.figure(figsize = (12,12))
	#ax = fig.add_subplot(111)
	#qp = rotateinitialmanifold(a_orb,b_orb,-t,qp)
	colormap = plt.get_cmap("Dark2")
	ax3.plot(eta,ImS,'o',zorder = 2,color = colormap( int(np.floor(count/2) +3) ),markersize = 8)
	ax4.plot(xi2,eta_abs,'o',zorder = 2,color = colormap( int(np.floor(count/2) + 3 ) ),markersize = 8)
	ax5.plot(position_onrealx,position_onrealy,'o',color = colormap(int(np.floor(count/2) + 3)))
	ax5.set_xlabel(r'$q$',fontsize = 25, labelpad = 22)
	ax5.set_ylabel(r'$p$',fontsize = 25, labelpad = 22,rotation = "horizontal")
	#ax3.plot(eta_all,ImS_all,"o",zorder = 1,color = colormap(int(count/2) ))
	phase = np.loadtxt('scatteringphase_original_3.txt')
	ax5.plot(phase[0,0:80000],phase[1,0:80000],',k',zorder = 2)
	#ax3.set_xlabel(r"$|\eta|$")
	#x3.set_ylabel(r"$|{\rm Im}S|$")
	ax3.set_xlabel(r'$|\eta|$',fontsize = 33)
	ax3.set_ylabel(r'$ | {\rm Im}(S)|$',fontsize = 33, labelpad = 22)
	ax3.set_ylim(0.0,0.06)
	ax3.set_xlim(0.7,2.0)
	...
	#	print(periods[i])
	#	drawingpoint(qp[:,i],periodicpoint[:,i],periods[i])
	return allpoint
...
def drawingpoint(point,periodicpoint,period):
	global actions, iterates
	print(periodicpoint)
	...
	for i in range(300):
		...
		#points[2] = np.append(points[2], (np.imag(point[0]) ** 2 + np.imag(point[1]) ** 2) ** 0.5 )
		points[2] = np.append(points[2], (np.imag(point[0])  ))
		point = cmap.U(point)
		print(point)
	...
	ax.plot(phase[0,0:80000],phase[1,0:80000],',b',zorder = 1)
	#for i in range(1):
	#	fig  = plt.figure(figsize = (12,12))
	#	ax = fig.add_subplot(121, projection = '3d')
	#	ax2 = fig.add_subplot(122)
	#	ax.view_init(10,i/10 *360)
	ax.plot(points[0],points[1],points[2],'-o',zorder = 2,color = "red" )

	ax.plot(initpoint[0],initpoint[1],initpoint[2],'-o',zorder = 2,color = "Green",markersize = 10 )
	...
	ax.set_zlabel(r"${\rm Im}(q)$", fontsize = 26, labelpad = 30,fontname ="IPAexGothic")
	#ax.set_zlim(0,0.05)
