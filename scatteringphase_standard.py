...
def U(qp):
    ...
    qp_new[0] =   (qp[0] + qp[1] - dotV(qp[0]) )
    qp_new[1] = (qp[1] -  dotV(qp[0])  )
...
def main():
    seed = 400
    q =  (np.random.random(seed) - 0.5) * 6
    p =  (np.random.random(seed) - 0.5 )  * 6
    traj = [np.array([])] * 2 
    for i in range(tmax):
        print(i)
        q,p = U([q,p])
    ...
    for l in range(1,2):
        ...
        plt.tight_layout()
        np.savetxt("scatteringphase_standard_001.txt",traj)
    plt.show()
